...
public class OtherClassScanner {
     ...
     * 如果包名指定为空，不跟点
     */
    protected final String packageNameWithDot;
    /**
     * 包路径，用于文件中对路径操作
     */
    protected final String packageDirName;
    /**
     ...
     * 过滤器
     */
    protected final Filter<Class<?>> classFilter;
    /**
     * 编码
     */
     ...
     */
    public OtherClassScanner(String packageName) {
     ...
     */
    public OtherClassScanner(String packageName, Filter<Class<?>> classFilter) {
     ...
     * @param charset     编码
     */
    public OtherClassScanner(String packageName, Filter<Class<?>> classFilter, Charset charset) {
    ...
    public static Set<Class<?>> scanAllPackage(String packageName, Filter<Class<?>> classFilter) {
        return new OtherClassScanner(packageName, classFilter).scan(true);
    }
    ...
    public static Set<Class<?>> scanPackage(String packageName, Filter<Class<?>> classFilter) {
        return new OtherClassScanner(packageName, classFilter).scan();
    }
    ...
    public Set<Class<?>> scan(boolean forceScanJavaClassPaths) {
        ...
        //try {
            ...
            for (URL url : //((URLClassPath)field.get(classLoader)).getURLs()){
     ...
     */
    protected void scanJavaClassPaths() {
     ...
     */
    protected void scanFile(File file, String rootDir) {
        if (file.isFile()) {
     ...
     * @param jar jar包
     */
    protected void scanJar(JarFile jar) {
        String name;
     ...
     * @param className 类名
     */
    protected void addIfAccept(String className) {
    ...
    protected void addIfAccept(Class<?> clazz) {
        if (null != clazz) {
     ...
     * @return 包名之前的部分
     */
    protected String subPathBeforePackage(File file) {
